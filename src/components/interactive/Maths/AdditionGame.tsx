...
export const AdditionGame: React.FC = () => {
  ...
  return (
    <div className="max-w-md mx-auto bg-app-card p-6 rounded-2xl border border-app-border shadow-xl text-center">
      <h3 className="text-2xl font-bold text-white mb-2">➕ Addition</h3>
      <p className="text-gray-400 text-sm mb-4">Add the groups together! (Score: {score})</p>
      ...
      <div className="flex justify-center items-end gap-6 mb-6">
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="flex gap-1">
            {Array.from({ length: current.b }).map((_, i) => <span key={i} className="text-3xl">{current.emoji}</span>)}
          </div>
        </div>
        <span className="text-4xl text-white">=</span>
        <span className="text-4xl text-white">?</span>
