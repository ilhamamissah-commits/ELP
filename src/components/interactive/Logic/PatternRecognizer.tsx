...
const CHALLENGES: PatternChallenge[] = [
  ...
  },
  {
    ...
    explanation:
      'The pattern mirrors itself: Red, Blue, Green, Green, Blue, Red, Blue...',
...
export const PatternRecognizer: React.FC<PatternRecognizerProps> = ({
  onComplete,
}) => {
  ...
  const [progress, setProgress] = useState<Record<number, PatternProgress>>({});
  ...
  const levelChallenges = useMemo(
    () => CHALLENGES.filter((challenge) => challenge.level === levelNumber),
    [levelNumber]
  );

  const completedChallenges = useMemo(
    () => Object.values(progress).filter((item) => item.mastered).length,
    [progress]
  );
  ...
  /* Auto-read the challenge title + objective + pattern when it changes */
  useEffect(() => {
    if (!autoReadEnabled || !current || isComplete || hasFinished) return;

    const timer = window.setTimeout(() => {
      speak(`${current.title}. ${current.objective}. What comes next?`);
    }, 400);

    return () => window.clearTimeout(timer);
  }, [challengeIndex, current, isComplete, hasFinished, speak, autoReadEnabled]);

  /* Read the hint when it opens */
  useEffect(() => {
    if (showHint && current && !answerChecked) {
      speak(current.hint);
    }
  }, [showHint, current, answerChecked, speak]);

  /* Announce journey completion */
  useEffect(() => {
    if (!isComplete || !correct) return;

    speak(
      `Pattern Journey Complete! You solved all ${CHALLENGES.length} pattern challenges with ${score} points.`,
    );
  }, [isComplete, correct, score, speak]);

  const updateProgress = (challengeId: number, wasCorrect: boolean) => {
    setProgress((previous) => {
      ...
      return {
        ...
        [challengeId]: {
          ...
          mastered: existing.mastered || wasCorrect,
        },
      };
    });
  };
  ...
  const handleSelect = (option: string) => {
    ...
    if (isAnswerCorrect) {
      ...
      speak(
        'Not quite. Think about the sequence again. Look for what repeats or changes each time.',
      );
    }
  };
  ...
  const resetGame = () => {
    ...
    speak("Let's practise patterns again!");
  };
  ...
  if (hasFinished) {
    return (
      ...
      >
        ...
        <h3 className="text-2xl font-bold text-white mb-2">Pattern Master!</h3>

        <p className="text-gray-400 mb-6">
          You completed the Pattern Recognition journey.
        </p>
    ...
    <div className="max-w-2xl mx-auto bg-app-card p-5 sm:p-6 rounded-2xl border border-app-border shadow-xl">
      ...
      <div className="flex justify-between items-start gap-4 mb-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Discover patterns, sequences and what comes next.
          </p>
        </div>
      ...
      <div className="bg-gray-800/50 rounded-xl p-4 mb-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">{current.objective}</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-5">
        <div className="flex justify-between text-xs mb-1">
          ...
          <span className="text-gray-300">{masteryPercentage}% mastery</span>
        </div>
      ...
      <div className="mb-6">
        ...
        <div className="flex justify-center items-center gap-2 flex-wrap">
          <AnimatePresence mode="popLayout">
            {current.pattern.map((item, index) => (
              <motion.div
      ...
      <div className="flex justify-center gap-3 flex-wrap">
        {current.options.map((option) => {
          ...
          const isCorrectAnswer = answerChecked && option === current.next;

          const isWrongSelection = answerChecked && isSelected && !correct;

          return (
            <motion.button
              key={option}
              whileHover={!answerChecked ? { scale: 1.08 } : undefined}
              whileTap={!answerChecked ? { scale: 0.94 } : undefined}
      ...
      {answerChecked && (
        ...
        >
          <div
            className={`font-bold ${correct ? 'text-green-400' : 'text-red-400'}`}
          >
            {correct ? (
              <>
          ...
          {correct && showExplanation && (
            <p className="text-sm text-gray-300 mt-2">{current.explanation}</p>
          )}

          {!correct && (
            <p className="text-sm text-gray-400 mt-2">
              Try another option. Look for what repeats or changes each time.
            </p>
          )}
        </motion.div>
      )}
      ...
      {isComplete && correct && (
        ...
        >
          ...
          <p className="text-sm text-gray-400 mt-1">
            You solved all {CHALLENGES.length} pattern challenges.
          </p>
      ...
      <div className="mt-5 pt-4 border-t border-gray-800">
        <div className="flex justify-between text-xs text-gray-500">
          <span>
            ...
            {levelChallenges.length !== 1 ? 's' : ''} in this level
          </span>
