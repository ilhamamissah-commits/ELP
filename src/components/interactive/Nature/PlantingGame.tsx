...
const STAGES: readonly PlantingStage[] = [
  {
    ...
    instruction: 'Dig a small hole in the soil.',
    emoji: '🕳️',
  ...
  },
  {
    ...
    instruction: 'Put the little seed in the hole.',
    emoji: '🌰',
  ...
  },
  {
    ...
    instruction: 'Cover the seed gently with soil.',
    emoji: '🌱',
  ...
  },
  {
    id: 'water',
    title: 'Give it water',
    instruction: 'Water the seed gently.',
    emoji: '💧',
  ...
  },
  {
    id: 'grow',
    title: 'Watch it grow!',
    instruction: 'The seed grows into a little plant.',
    emoji: '🌱',
...
const PLANTING_ACTIVITY_ID = 'nature-planting-001';
...
export const PlantingGame: React.FC = () => {
  ...
  const progress = completed
    ? 100
    : Math.round(((step + 1) / TOTAL_STAGES) * 100);

  /**
   * Auto-read the current stage title + instruction whenever it changes.
   * Cleanup clears any pending auto-read and pending feedback timeout on unmount.
   */
  useEffect(() => {
    if (!autoReadEnabled || completed) return;

    const timer = window.setTimeout(() => {
      speak(`${currentStage.title}. ${currentStage.instruction}`);
    }, 400);

    return () => window.clearTimeout(timer);
  }, [step, currentStage, completed, speak, autoReadEnabled]);
  ...
  useEffect(() => {
    ...
    speak(
      'Wonderful! You grew a plant. Great job — you learned how a seed can grow into a plant.',
    );
  }, [completed, speak]);

  /**
   * Clean up delayed feedback timeout on unmount.
   */
  useEffect(() => {
    return () => {
  ...
  const handleAction = () => {
    ...
    timeoutRef.current = window.setTimeout(() => {
      ...
      if (step < TOTAL_STAGES - 1) {
        setStep((previous) => previous + 1);
        return;
      }

      /**
       * The learner completed the complete planting sequence.
      ...
      completeActivity({
        id: PLANTING_ACTIVITY_ID,
        score: 100,
        academyId: 'nature',
        domain: 'science',
        skillIds: PLANTING_SKILLS,
      });
    }, 900);
  };
  ...
  const resetGame = () => {
    ...
    speak("Let's plant a seed again!");
  };
        ...
        <div className="relative overflow-hidden px-6 pb-8 pt-8 text-center sm:px-8">
          ...
          <div className="mt-7 rounded-2xl border border-green-400/20 bg-green-400/10 p-4">
            <div className="flex items-center justify-center gap-2 text-green-300">
              ...
              <span className="font-bold">Planting complete!</span>
            </div>
          </div>
