...
const ITEMS: readonly RecyclingItem[] = [
  {
    ...
    name: 'Banana Peel',
    type: 'Organic',
    childDescription: 'A banana peel can go with food and plant waste.',
  },
  {
    id: 'plastic-bottle',
    emoji: '🥤',
  ...
  },
  {
    ...
    name: 'Cardboard Box',
    type: 'Recyclable',
    childDescription: 'Clean cardboard can be recycled.',
  },
  {
    id: 'apple-core',
    emoji: '🍎',
    name: 'Apple Core',
    type: 'Organic',
    childDescription: 'An apple core can go with food and plant waste.',
  },
  {
    id: 'battery',
    emoji: '🔋',
  ...
  },
  {
    ...
    name: 'Tin Can',
    type: 'Recyclable',
    childDescription: 'Metal cans can often be recycled.',
  },
  {
    id: 'newspaper',
    emoji: '🗞️',
    name: 'Newspaper',
    type: 'Recyclable',
    childDescription: 'Clean paper can be recycled.',
  },
  {
    id: 'chicken-bone',
    emoji: '🍗',
    name: 'Chicken Bone',
    type: 'Organic',
    childDescription: 'Food scraps can go with organic waste.',
  },
] as const;
...
export const RecyclingGame: React.FC = () => {
  ...
  const progress = done ? 100 : Math.round((index / TOTAL_ITEMS) * 100);
  ...
  /**
   * Auto-read the current item whenever it changes.
   */
  useEffect(() => {
    if (!autoReadEnabled || done || !item) return;

    const timer = window.setTimeout(() => {
      speak(
        `${item.name}. Where does this belong? ${item.childDescription}`,
      );
    }, 400);

    return () => window.clearTimeout(timer);
  }, [index, item, done, speak, autoReadEnabled]);
  ...
  useEffect(() => {
    ...
    speak(
      `Amazing sorting! You are a recycling hero. You earned ${score} points.`,
    );
  }, [done, score, speak]);
  ...
  const handleSort = (bin: WasteType) => {
    ...
    if (bin === item.type) {
      ...
      speak(`Great job! ${item.name} goes in the ${bin} bin.`);
      ...
      feedbackTimeoutRef.current = window.setTimeout(() => {
        ...
        if (index < TOTAL_ITEMS - 1) {
          ...
          /*
           * Record the completed activity in the ELP progress engine.
           * This activity belongs to: Nature & Agriculture → Science
           */
          completeActivity({
  ...
  const resetGame = () => {
    ...
    speak("Let's play Recycling Hero again!");
  };
      ...
      >
        <div className="px-6 pb-8 pt-8 text-center sm:px-8">
          ...
          <p className="mt-2 text-sm leading-6 text-gray-400 sm:text-base">
            Amazing sorting! You helped learn where different things belong.
          </p>
          ...
          <div className="mt-6 rounded-2xl border border-green-400/20 bg-green-400/10 p-4">
            <div className="flex items-center justify-center gap-2 text-green-300">
              ...
              <span className="font-bold">Every little action helps!</span>
            </div>
          </div>
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="rounded-full border border-app-border bg-gray-900/50 p-2 transition hover:bg-gray-800"
            >
              <Volume2
                className={`h-4 w-4 ${
      ...
      <div className="px-5 pb-6 sm:px-7">
        ...
        <div className="grid grid-cols-3 gap-2.5 sm:gap-3">
          {BINS.map((bin) => {
            ...
            const isCorrect =
              feedback === 'correct' && item.type === bin.id;

            const isWrong = feedback === 'tryAgain' && isSelected;
        ...
        {item.type === 'Hazardous' && (
          ...
          >
            ...
            <span>
              Some things need special care. Always ask a grown-up for help
              with batteries.
            </span>
          </motion.div>
        )}
        ...
        <div className="mt-5 flex items-center justify-between rounded-2xl border border-app-border bg-gray-900/40 px-4 py-3">
          <div>
            <div className="text-xs font-bold text-gray-500">Your score</div>
