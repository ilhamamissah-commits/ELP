...
export interface LessonItem {
  ...
  /**
   * Whether this lesson is the best next lesson
   * for the learner based on demonstrated mastery.
   */
  recommended?: boolean;

  /**
   * Optional explanation when a lesson cannot
   ...
   *
   * This is the ELP curriculum level, NOT the learner's age.
   */
  level?: number;

  /**
   * Optional domain/skill represented by this lesson.
   */
  domain?: string;

  /**
   * Optional mastery score for this lesson.
   */
  mastery?: number;
}
...
interface SubjectLessonListProps {
  ...
  /**
   * Current overall ELP learner level.
   */
  currentLevel?: number;
}
...
export const SubjectLessonList: React.FC<
...
> = ({
  ...
  /*
   * Recommended lessons appear first.
   *
   * Within each group:
   * 1. Available lessons
          ...
          <section className="mb-7">
            ...
            <div className="space-y-3">
              {availableLessons.map((lesson, index) =>
                renderLesson(
                  lesson,
                  index + recommendedLessons.length
                )
              )}
            </div>
          </section>
