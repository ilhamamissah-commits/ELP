...
const getRecommendation = (skillId: string) => {
  ...
  if (
    skill.includes('letter') ||
    skill.includes('phonics') ||
    skill.includes('sound') ||
  ...
  if (
    ...
    skill.includes('count') ||
    skill.includes('addition') ||
    skill.includes('math')
  ) {
  ...
  if (
    skill.includes('science') ||
    skill.includes('observation')
  ) {
...
export const ParentPortal: React.FC<ParentPortalProps> = ({ onBack }) => {
  ...
  /*
   * The profile store is now the source of truth for the learner's
   * identity and overall ELP progression.
   */
  const profile = currentProfileId
    ? profiles[currentProfileId]
    : undefined;

  const childName = profile?.name || progressChildName || 'Learner';
  ...
  /*
   * Detailed activity-level mastery remains in useProgressStore.
   * This gives the parent both:
   *
   * 1. Learner-level/domain progress from useProfileStore
   * 2. Detailed skill performance from useProgressStore
   */
  const skillEntries = Object.values(skills);

  const totalSkills = skillEntries.length;

  const masteredSkills = skillEntries.filter(
    (skill) => skill.bestScore >= 80
  ).length;

  const practicingSkills = skillEntries.filter(
    (skill) => skill.bestScore >= 50 && skill.bestScore < 80
  ).length;

  const developingSkills = skillEntries.filter(
  ...
  const weaknesses = useMemo(
    () =>
      skillEntries
        .filter((skill) => skill.bestScore < 50)
  ...
  const handleUnlock = () => {
    ...
    // Development-only PIN.
    // Replace with secure parent authentication before production.
    if (passcode === '1234') {
      setIsUnlocked(true);
      setPasscode('');
      return;
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Learning progress and development overview
          </p>
        </div>
      ...
      {!isUnlocked ? (
        <div className="flex flex-col items-center gap-5 p-8 bg-[#1a1a1a] rounded-xl border border-gray-800">
          ...
          <div className="text-center">
            ...
            <p className="text-gray-400 text-sm mt-1">
              Enter the parent passcode to view {childName}'s learning analytics.
            </p>
          </div>
          ...
          <p className="text-[10px] text-gray-600">
            Parent authentication should be replaced with secure
            account-based authentication before production.
          </p>
        </div>
      ) : (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <Card className="p-4 border-l-4 border-amber-500">
              ...
              <p className="text-2xl font-bold text-amber-300 mt-1">
                {practicingSkills + developingSkills}
              </p>
            </Card>
          </div>
            ...
            <Card className="p-4">
              ...
              <div className="space-y-3">
                {[
                  ...
                  const score =
                    profileSkills[
                      key as keyof typeof profileSkills
                    ] ?? 0;
          ...
          <Card className="p-4">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Detailed performance across completed learning activities
                </p>
              </div>
            </div>
            ...
            {totalSkills === 0 ? (
              <div className="text-center py-5">
                <p className="text-sm text-gray-500">
                  No skill performance has been recorded yet.
                </p>
              </div>
            ) : (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <span
                            className={`text-xs font-semibold ${getMasteryColor(
                              skill.bestScore
                            )}`}
                          >
                            {skill.bestScore}%
                      ...
                      <span
                        className={`text-[10px] whitespace-nowrap ${getMasteryColor(
                          skill.bestScore
                        )}`}
                      >
          ...
          <Card className="p-4 border border-red-500/10">
            <div className="flex items-center gap-2 mb-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Areas that may benefit from additional practice
                </p>
              </div>
            </div>
            ...
            {weaknesses.length > 0 ? (
              ...
              <div className="flex items-center gap-2 p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/10">
                ...
                <p className="text-sm text-gray-300">
                  No major skill gaps detected. {childName} is
          ...
          <Card className="p-4">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Activities assigned through the learner's classroom
                </p>
              </div>
            </div>
            ...
            {assignments.length === 0 ? (
              ...
              <div className="space-y-2">
                {assignments.map((assignment) => (
                  <div
                    key={assignment.id}
                    className="flex items-center justify-between gap-3 bg-gray-800/70 p-3 rounded-lg"
                  >
          ...
          <div className="text-center pt-1">
            <p className="text-[10px] text-gray-600">
              Learning level is determined by demonstrated competency,
              not age. Foundational skills remain available for adaptive review.
            </p>
          </div>
        </div>
      )}
